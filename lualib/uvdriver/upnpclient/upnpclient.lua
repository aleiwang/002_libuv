...
local http      = require('uvdriver.http')
local urlparser = require('uvdriver.upnpclient.url')
...
function uPnPclient:discoverIGD()
    ...
    local ok, err_or_statu = skt:connect(self.url.host, self.url.port)

    if not ok then
	   return nil, "ERROR: failed to open tcp socket to " .. self.url.host .. ':' .. self.url.port .. " - " .. err_or_statu
    end

    local errno, name, port = skt:getsockname() 
    self.internalClient = name 
...
function uPnPclient:discover(search_target)
    ...
    local ok, status_or_err, len = udp:sendto("239.255.255.250", 1900, pkt_ssdp_discover) 
    assert(ok)

    -- Wait for reply
    -- TODO handle multiple replies
    local ok, discover_response_or_err = udp:read(1000) 
    if not ok then
	   return nil, "ERROR: No response to discovery packet: " .. tostring(discover_response_or_err)
    end

    local location = string.match(discover_response_or_err,"LOCATION:%s+(http://%S+)")

    if location == nil or location == "" then
	   return nil, "ERROR: Unable to parse location from discover response: " .. tostring(discover_response_or_err)
    end
    ...
    local dev_http_response, err = http.get(location)

    if err then
	   return nil, "ERROR: GET " .. location .. " FAILED: " .. err
    end

    -- Parse into a simple table tree
    self.deviceProfile = simple_xml_parse(dev_http_response)
...
function uPnPclient:SendSoapRequest(soap_body, action)
    ...
    -- Send request to IGD
    local dev_http_response, err = http.post(self.controlURL, soap_body, { headers = soap_headers })

    if err then
    	if err:match('^<s:Envelope') then
    	    local err = simple_xml_parse(err)
    	...
    	return nil, "GET " .. self.controlURL .. " ACTION: " .. action .. " FAILED: " .. err
    end

    self:debug(9, "dev_response.body: ", dev_http_response)

    -- Parse into a simple table tree
    local r = simple_xml_parse(dev_http_response)
